...
def multi_model_forecast(
    ...
    date_predict = pd.DataFrame(pd.date_range(start=y_train.ds.iloc[-1], periods=forecast_len, freq=freq), columns = ['ds'])

    to_return = {}

    assert forecast_len > len(y_test['y']) or 0, "Убедитесь, что длинна прогноза больше тестовой части"   
